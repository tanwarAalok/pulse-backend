...
export class GetPostController {
    public async posts(req: Request, res: Response) {
        ...
        res.status(HTTP_STATUS.OK).json({message: 'All posts found', posts, totalPosts});
    }

    public async postsWithImage(req: Request, res: Response) {
        const { page} = req.params;
        const skip: number = (parseInt(page) - 1) * PAGE_SIZE;
        const limit: number = parseInt(page) * PAGE_SIZE;
        const newSkip: number = skip === 0 ? skip : skip + 1;
        ...
        res.status(HTTP_STATUS.OK).json({message: 'All posts with images found', posts});
    }
}
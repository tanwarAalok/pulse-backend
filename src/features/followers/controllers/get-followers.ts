...
export class GetFollowersController {
    ...
    public async userFollowings(req: Request, res: Response) {
        ...
        const cachedFollowings: IFollowerData[] = await followerCache.getFollowersFromCache(`followings:${req.currentUser!.userId}`);

        const following: IFollowerData[] = cachedFollowings.length ? cachedFollowings :
